    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-4">
                ...
                <p className="text-gray-500 font-bold uppercase tracking-widest text-xs">Loading Certificates...</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-8 pb-20 max-w-7xl mx-auto">
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-4xl font-black text-white uppercase tracking-tighter">{t('certificates') || 'My Certificates'}</h1>
                    <p className="text-gray-400 font-bold mt-1">View, download, and share your verified achievements.</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {filteredCertificates.map((cert, i) => (
                    <motion.div
                        ...
                        className="glass rounded-[2.5rem] border border-white/5 overflow-hidden group hover:border-white/20 transition-all flex flex-col pt-2 bg-gradient-to-br from-black/40 to-transparent"
                    >
                        {/* Certificate Ribbon */}
                        <div className="absolute top-4 right-4 z-10 w-12 h-12 bg-gradient-to-br from-yellow-400 to-amber-600 rounded-full flex items-center justify-center text-white shadow-lg shadow-yellow-500/20 border-[3px] border-dark">
                            <FiAward className="text-2xl" />
                        </div>

                        <div className="p-6 pb-0">
                            <div className="w-full aspect-[4/3] rounded-[1.5rem] overflow-hidden relative border border-white/10 shadow-2xl bg-white flex flex-col items-center justify-center text-center p-4">
                                <div className="absolute inset-x-0 top-0 h-4 bg-primary" />
                                <h4 className="text-xs font-black text-gray-400 uppercase tracking-[0.2em] mb-4">Arqam Academy</h4>
                        ...
                        <div className="p-6 flex-1 flex flex-col justify-between mt-4">
                            <div>
                                ...
                                <div className="flex justify-between items-center text-xs font-bold text-gray-400 mb-4 pb-4 border-b border-white/5">
                                ...
                                <div className="flex justify-between items-center text-[10px] font-black text-gray-500 uppercase tracking-widest">
                                    ...
                                    <span className="bg-white/5 px-2 py-1 rounded">ID: {cert.credentialId}</span>
                                </div>
                            </div>

                            <div className="flex gap-2 mt-6">
                                <button className="flex-1 py-3 bg-white/5 hover:bg-white/10 text-white font-black text-xs uppercase tracking-widest rounded-xl transition-all border border-white/10 flex items-center justify-center gap-2">
                                    <FiDownload /> Download
                                </button>
                ...
                {filteredCertificates.length === 0 && (
                    <div className="col-span-full py-20 text-center glass rounded-[3rem] border border-dashed border-white/10">
                        <FiAward className="mx-auto text-5xl text-gray-700 mb-4" />
                        <p className="text-gray-500 font-bold uppercase tracking-widest text-xs">No certificates found.</p>
                        <p className="text-gray-600 font-medium text-sm mt-2">Complete courses and tracks to earn verified certificates.</p>
                    </div>
                )}
            </div>
        </div>
