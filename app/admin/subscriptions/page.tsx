    ...
    return (
        <div className="space-y-8">
            <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-black text-white uppercase tracking-tighter">Subscriptions & Plans</h1>
                    <p className="text-gray-400 font-medium text-sm mt-1">Manage subscription packages, pricing, and duration.</p>
                </div>
            ...
            <div className="grid grid-cols-3 gap-4">
                <div className="glass rounded-2xl p-6 border border-white/5 text-center">
                    ...
                    <p className="text-xs font-black text-gray-500 uppercase tracking-widest">Total Plans</p>
                    <p className="text-3xl font-black text-white">{plans.length}</p>
                </div>
                <div className="glass rounded-2xl p-6 border border-white/5 text-center">
                    <FiUsers className="text-green-400 text-2xl mx-auto mb-2" />
                    <p className="text-xs font-black text-gray-500 uppercase tracking-widest">Total Subscribers</p>
                    <p className="text-3xl font-black text-white">{plans.reduce((a, p) => a + p.activeUsers, 0)}</p>
                </div>
                <div className="glass rounded-2xl p-6 border border-white/5 text-center">
                    <span className="text-yellow-400 text-2xl mx-auto mb-2 block">$</span>
                    <p className="text-xs font-black text-gray-500 uppercase tracking-widest">Monthly Revenue</p>
            ...
            <div className="grid md:grid-cols-3 gap-6">
                {plans.map((plan, i) => (
                    <motion.div
                        ...
                        className={`glass rounded-2xl border p-8 flex flex-col relative overflow-hidden ${i === 1 ? 'border-primary/40 shadow-[0_0_30px_rgba(var(--primary),0.1)]' : 'border-white/10'}`}
                    >
                        {i === 1 && (
                        ...
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <button className="p-2 rounded-xl text-gray-500 hover:text-white hover:bg-white/10 transition-colors">
                                <FiEdit2 className="text-sm" />
                            </button>
                        </div>
                        <div className="mb-6">
                            <span className="text-4xl font-black text-white">{plan.price === 0 ? 'Free' : `$${plan.price}`}</span>
                            {plan.price > 0 && <span className="text-gray-500 font-bold text-sm ml-1">{plan.period}</span>}
                        </div>
                        <ul className="space-y-3 flex-1 mb-6">
                            {plan.features.map((f, fi) => (
                                <li key={fi} className="flex items-center gap-3 text-gray-300 font-medium text-sm">
                        ...
                        <div className="border-t border-white/5 pt-4 mt-auto">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Active Subscribers</p>
                                    <p className="text-white font-black text-xl">{plan.activeUsers}</p>
                                </div>
                                <button className="p-2 rounded-xl text-red-500/50 hover:text-red-400 hover:bg-red-400/10 transition-colors">
